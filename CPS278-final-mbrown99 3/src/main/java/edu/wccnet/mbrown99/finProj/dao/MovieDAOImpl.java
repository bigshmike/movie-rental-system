...
public class MovieDAOImpl implements MovieDAO {
	...
	public Movie getMovieByInfo(String movieInfo) {
		// TODO Auto-generated method stub
		Session session = sessionFactory.getCurrentSession();
		Query<Movie> query = session.createQuery("from Movie where movie_info like :movieInfo ", Movie.class);
		query.setParameter("movieInfo", movieInfo);
		return query.getSingleResult();
	}
	
	@Override
	public void returnMovie(int id) { // use rentalId not movieId
		Session session = sessionFactory.getCurrentSession();
